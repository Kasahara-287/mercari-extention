...
画像読み取り機能はないため、画像に関する評価は行わないでください。
「無言取引NG」等の文言があった場合は、ほかフリマサイトと取引を同時に行っている可能性があります。その場合は「他サイトにも出品している可能性があり取引時のトラブルに発展しやすい」と記載し、危険度を上げてください。
「無言取引NG」等の文言がなかった場合に無言取引等に関する文章の記載は一切しないでください。「無言取引NG」等の文言がないというのはリスクにはなりません。またそれらの文言はないのが当たり前であるため、ないからと言って危険度を下げる、または文章出力するのはしないでください。
できるだけ詳しく記載してください。
危険度の初期値を50%でお願いします、計算方式は危険であれば足し算、安全そうであれば引き算でお願いします、危険度が(0%-40%)の際は○○％危険度:低、(41%-70%)ではOO%危険度:中、(71%-100%)ではOO%危険度:高とOO%のOOにはその時の危険度の数値に合わせてを表記してください。
出力は次の形式でお願いします：

[OO%のためこの商品の危険度:高,OO%のためこの商品の危険度:中,OO%のためこの商品の危険度:低]
